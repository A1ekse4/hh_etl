...
| ---|---|
| **Базовый ELT** — инкрементальный забор данных, сохранение в БД | INSERT новых, UPDATE изменённых (по `row_hash`), мягкий DELETE удалённых вакансий |
| **Логирование** | `logging` с форматтером, вывод в файл `logs/hh_etl.log` и консоль, уровень настраивается в `config.yaml` |
| **Обработка ошибок — ретраи** | `tenacity`: 5 попыток с экспоненциальным backoff (1–30 сек) на каждый HTTP-запрос |
| **Конфигурация вынесена из кода** | Hydra + `conf/config.yaml`; секреты (пароль БД и др.) — через переменные окружения `.env` |
| **Уникальный `etl_id`** | UUID генерируется на старте, прокидывается в логи (`etl_id=...` в каждой строке), в таблицу `etl.hh_vacancies` и в таблицу истории `etl.hh_etl_runs` |
| **Airflow DAG** | `dags/hh_etl_dag.py` — DAG запускает ноутбук через `papermill`, расписание `0 6 * * *` (ежедневно в 06:00 UTC) |
| **Data lake в колоночном формате** | Parquet + Snappy (или zstd/lz4/gzip) в `data_lake/hh_vacancies/year=/month=/day=/`, партиционирование по дате запуска |
